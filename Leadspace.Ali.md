...
**最后更新**：2026-08-21（小绿盒企微外推已上线生产 · 2026-08-21 验收通过）
...
| 路径 | 角色 | 功能 |
|---|---|---|
| `/xlv/admin/inventory` | DIRECTOR | **库存看板**（默认 Tab）、期初盘点、**新增入库**、**回拨机具**、**划拨下级**、**撤机**（移机明细表）、待收货确认 |
| `/xlv/inventory` | MANAGER | **库存看板**、待收货确认、分给队员、**撤机**（移机明细表） |
...
**撤机（增量 A · 移机明细表）**：运营侧 Excel 文件名虽叫「移机明细」，业务含义是 **撤机**（从门店撤下、不再按已铺设考核），**不是**调拨、**不是**回事业部总库。上传后 **不立即改库存/运营**，而为每台 SN 创建 **待确认撤机单**，向 **铺设归属人**（经理铺→经理账号，队员铺→队员账号）发站内通知，正文含 **SN + 门店名**；归属人 **同意** 后执行：`deployed` → 原铺设人库存，并 **清零运营考核态**；**拒绝** 则不变。同 SN 待确认期间不可重复提交；同 SN 多行取 **进件日期最新** 一条。经理 **只能撤本团队** SN；DIRECTOR 可撤全部。
...
**撤机确认 API**：`POST /api/xlv/inventory/withdraw-requests/{id}/respond`，body `{ "action": "approve" | "reject" }`；仅小绿盒归属人账号。通知类型 `withdraw_pending`，入口 `/xlv/notifications`（同意/拒绝按钮）。
...
- [x] **企微外推**：`outbound-notifier.ts` — 队员跟进完成、撤机待确认旁路推群（`XLV_OUTBOUND_WEBHOOK_URL`）
...
| **小绿盒 5 · 库存** | 入库/回拨机具/划拨下级/撤机二次确认、库存看板合规率、队员未铺芯片、XLV 性能优化；生产 **待本次部署** 后导历史移机明细（方案 B，不需 backfill 脚本） |
