...
**最后更新**：2026-09-08（小绿盒看板默认该催谁 · **已部署** `__HASH__`）
...
| 小绿盒看板 | `/xlv/alerts` 六宫格：**已达标 / 达标率按全部已铺设**（往月未达标当月继续考核，不限首笔月）；**本月/上月拓展**只统计该月新铺；沉睡 / 单笔沉默为当前状态。列表**默认全部已铺设**，点「本月拓展」才按首笔月筛。日期区间与中国日历日对齐（跟进按北京时间）。**考核中**卡片展示当月成绩 + 距目标缺口，至今累计只在详情。`/xlv/board` 顶栏合规/已铺设与距 90% 差几台。看重点默认 **该催谁**（待跟进多的在前），另有 **看健康** / **跟进成效**。卡片写「合规率」与「n/m 台合规」。队员行合并账号 + 同名未绑定。**`/xlv/admin/companies` 分公司排名看板** |
...
| `/xlv/board` | **团队看板**：顶栏合规/已铺设与距 90% 差几台。默认 **该催谁**；看重点还有 **看健康** / **跟进成效**。卡片「合规率」「n/m 台合规」。队员行合并账号 + 同名未绑定 |
| `/xlv/managers/[managerKey]` | 经理下队员名单；默认 `?sort=` = 该催谁；`?sort=compliance\|wake_rate`；`?status=` 考核筛选 |
