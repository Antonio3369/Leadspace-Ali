...
**最后更新**：2026-08-22（分公司看板 + 企微分流 · 待部署；运维告警 → 运维小群 `OPS_ALERT_WEBHOOK_URL` · **已部署** `38bb355`）
...
| 小绿盒数据上传企微 | 任意小绿盒导入 **SUCCESS/PARTIAL** → **运维小群** `OPS_ALERT_WEBHOOK_URL`（文件名、上传人、摘要 + 导入页链接） |
...
生产 https://ali.orblead.com。已上线：N7 底栏、队员开号、V1 关单、设备挂靠、设备搜索；**微信小绿盒**（沉睡回访、今日待办、关单回告、**运维告警运维小群**）。  
**待部署 / 本批**：分公司排名看板、企微分流（导入→运维 / SN归属汇总→负责人）、通知分栏与按 type 已读、撤机通知停用。
