...
**最后更新**：2026-09-03（小绿盒：缺首末笔时从累计/快照补回日期，导入不再用空值覆盖 · **已部署** `92c626d`）
...
| 小绿盒 SN 归属导入 | 按 **400 台一批**写设备/库存；Excel **只解析有内容的格子**（整列格式刷到百万行时不再按矩形分配内存）；考核只重算新建/换商户。合表可整份上传，**不是** 1400 行上限。2026-08-28 曾因 dense 解析 + 逐台写库存把 2GB 容器 OOM Kill，表现为「服务重启导致导入中断」 |
