...
**最后更新**：2026-08-25（小绿盒移机后不展示旧店交易 · SN 搜索编码 · F1 自动推运维小群 · **已部署** `c50cbe8`）
...
| 小绿盒移机展示 | 同 SN 换商户后，设备列表/详情标 **移机到新店**，并显示原门店；依赖 SN 归属成功写入的换商流水。列表「考核起算」为换商户日。**末笔早于考核起算的视为旧店交易，新店卡片不展示末笔/累计/沉睡**；详情交易趋势只保留考核起算及之后的点 |
...
生产 https://ali.orblead.com。已上线：N7 底栏、队员开号、V1 关单、设备挂靠、设备搜索；**微信小绿盒**（沉睡回访、今日待办、关单回告、运维告警运维小群、**分公司排名看板**、企微分流；**导入失败/中断推运维小群**、导入页三表成功失败一目了然、**移机到新店**、考核将到期点开看完整名单 · `f6a0468`；**列表/看板不再拉全量快照** · `03c79a3`；**移机后考核从换商户日起算** · `1b26649`；**移机后不展示旧店交易 / SN 搜索编码 / F1 部署成功推运维小群** · `c50cbe8`）。  
