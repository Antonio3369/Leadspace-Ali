...
**最后更新**：2026-09-08（小绿盒：队员排行合并同名设备 + 看板「看健康/该催谁/跟进成效」 · **已部署** `__HASH__`）
...
| 小绿盒双表导入 | 管理员 `/xlv/admin/import`：**先**运营原始表 → **再**组织名册 → **再** SN 归属；后台任务 + 轮询；导入完成展示**摘要**（列匹配、日期范围、重复合并、未匹配姓名）。① 同时认微信运营表（统计日期 + 累计*）和**乐刷渠道原表**（`0902_公司名_编号.xlsx`，「累积活跃用户数」；无统计日期时用文件名补；「昨日*」当该统计日增量）。指标列只做前缀匹配，**笔数不得吃进金额列** |
| 小绿盒 SN 归属导入 | 按 **400 台一批**写设备/库存；Excel **只解析有内容的格子**（整列格式刷到百万行时不再按矩形分配内存）；考核只重算新建/换商户。合表可整份上传，**不是** 1400 行上限。批量 UPDATE 的 VALUES **金额列须显式 float8**（用户/笔数为 int）：首行金额为 0 时 Postgres 否则把整列推断成 integer，`20942.8` 报 `22P02`，整表失败须重传。设备上**已有运营累计时，归属表累计不得覆盖**（避免金额被当成笔数）。终身已满 20 用户 + 300 笔且未移机时回写 `qualified`。2026-08-28 曾因 dense 解析 + 逐台写库存把 2GB 容器 OOM Kill，表现为「服务重启导致导入中断」 |
...
| 小绿盒考核 | 自然月增量：**+20 用户 + 300 笔**（两项须同时满足）；首笔交易月为装机月；**2027 年 3 月（含）前任意自然月达标即可**，过完 2027-03 仍未达标才 `invalid`；**月成绩 = 该自然月内最后截面「累计用户/笔数」减月前基线**（对齐微信运营表累计列；交易趋势图仍按收款日展示逐日笔数）；装机月无截面时可回退设备累计；状态 `qualified` / `in_progress` / `invalid`（`xlv-rules.ts`）。**同 SN 换商户（移机）后考核窗口从 `relocatedAt` 起算**，不再沿用旧商户首笔；微信解绑后的累计只计入新店窗口。本月拓展仍按微信 `firstTxnDate`。运营表漏填首末笔时：导入**不得用空值覆盖**已有日期；打开设备详情用快照/统计日补回（有累计则首末笔可落到该统计日）。仅一条截面且无当日/昨日列时，趋势用累计当作装机日活动；**已有昨日/当日笔数时不得把终身累计填进当日用户**（否则装机月成绩被减成 0） |
...
| 小绿盒看板 | `/xlv/alerts` 六宫格：**已达标 / 达标率按全部已铺设**（往月未达标当月继续考核，不限首笔月）；**本月/上月拓展**只统计该月新铺；沉睡 / 单笔沉默为当前状态。列表**默认全部已铺设**，点「本月拓展」才按首笔月筛。日期区间与中国日历日对齐（跟进按北京时间）。**考核中**卡片展示当月成绩 + 距目标缺口，至今累计只在详情。`/xlv/board` 经理排行 → 队员：账号设备与**同名未绑定**设备合并为一行（与点进设备页同一批）；看板「考核中」= 近日有动销的考核中（不含沉睡/单笔沉默）。看重点三档：**看健康**（合规率）/ **该催谁**（待跟进，非荣誉排名）/ **跟进成效**（唤醒率）；旧 `?sort=follow_up` 落到跟进成效。**`/xlv/admin/companies` 分公司排名看板** |
| 小绿盒设备库存 | `/xlv/admin/inventory`（Admin）/ `/xlv/inventory`（经理）：**库存看板**（收到 / 已铺设 / 剩余 / 铺设率 + **合规率** 供补货决策）；入库 / 划拨 / 期初；**期初默认跳过已有库存账**（不覆盖已铺设/经理库/队员库，可取消勾选以覆盖改经理名）；**移机撤机明细暂隐藏**，撤机改由 **SN 归属换商户** 推断；与团队看板分离，见 §6.2d |
| 小绿盒今日待办 | `/xlv`（首页）：P0 **优先催办**、P1 **疑似沉睡**、P2 **考核将到期**（政策窗口至 2027-03，剩≤15天仍未达标）。点「考核将到期」展开**这批设备完整名单**（不跳设备页「考核中」全量） |
...
| 小绿盒移机展示 | 同 SN 换商户后，设备列表/详情标 **移机到新店**，并显示原门店；依赖 SN 归属成功写入的换商流水。列表「考核起算」为换商户日。**末笔早于考核起算的视为旧店交易，新店卡片不展示末笔/累计/沉睡**；详情交易趋势只保留考核起算及之后的点。考核缺口也不用微信 SN 残量（如 8 用户/8 笔）去减目标，改为「装机月尚未收款 · 目标 20 用户 + 300 笔」 |
| 小绿盒数据上传企微 | 任意小绿盒导入 **SUCCESS/PARTIAL** → **运维小群**；**FAILED / 重启中断** 同样推运维小群（未成功必须重传，不能当已导入）。名册写完为 **SUCCESS**（下一步去人员归属核对同步），「部分成功」只留给缺列等真问题；企微名册文案为「新建/更新名册行」不是「设备新增 0」 |
...
| 小绿盒列表不拉全量快照 | 达标状态仍用落库 `qualificationStatus`；**考核中**列表为算当月成绩会拉该页 SN **上月起**快照（不拉全库全历史）。看板唤醒只拉跟进日之后；**设备详情**仍拉该 SN 快照。每日绩效只统计所选日期范围内关单 |
| 小绿盒设备搜索 | 设备页 / 回访搜**商户名 / SN / 队员 / 经理**；有搜索词时**不限宫格、不限已铺设**。SN 均含 `*`，写入 `?q=` 须编码为 `%2A`，否则商户名能搜到、SN 搜不到 |

### 1.2 本阶段停在哪里

生产 https://ali.orblead.com。已上线：N7 底栏、队员开号、V1 关单、设备挂靠、设备搜索；**微信小绿盒**（沉睡回访、今日待办、关单回告、运维告警运维小群、**分公司排名看板**、企微分流；**导入失败/中断推运维小群**、导入页三表成功失败一目了然、**移机到新店**、考核将到期点开看完整名单 · `f6a0468`；**列表/看板不再拉全量快照** · `03c79a3`；**移机后考核从换商户日起算** · `1b26649`；**移机后不展示旧店交易 / SN 搜索编码 / F1 部署成功推运维小群** · `c50cbe8`；**移机后考核不用旧店残量** · `96b682c`；**期初默认可跳过已有库存账** · `9ccf023`；**SN 归属导入降内存（收窄空表 + 分批写库存）** · `2c0c595`；**名册导入成功不再误标部分成功** · `9d80d3c`；**考核窗口延至 2027-03** · `5a3250d`；**设备详情缺首笔日期时考核进度不再写「尚未进入考核窗口」** · `0783bd6`；**缺首末笔时从累计/快照补回日期，导入空值不再覆盖** · `92c626d`；**SN 归属导入金额小数不再整表失败** · `8978e85`；**乐刷原表可直接导① + 笔数/金额防串列 + SN 归属不覆盖运营累计** · `b19f91b`；**考核中列表改当月成绩+距目标缺口，至今累计只在详情** · `3aa8232` / `541d8e2`；**达标按已铺设、列表默认不按拓展月切开** · `159ff70`；**队员排行合并同名未绑定设备、看板考核中对齐设备页、看重点三档** · `__HASH__`）。  
...
| `/xlv/alerts` | **设备**（全量列表）：顶部**两行六宫格**（所选月拓展 → 已达标 → 达标率 → 单笔沉默 → 沉睡 → 唤醒率）+ **本月/上月**（`dateFrom`/`dateTo`，默认本月）；点击可下钻列表或跳转回访情况；`?expand=month`、`?alert=`、`?status=`、`?manager=`、`?operator=`、`?q=`；指标 API `/api/xlv/dashboard/pulse`；**负责人选经理后，队员下拉仅该团队**；默认列表**不含剩余库存** |
...
| `/xlv/board` | **团队看板**：经理 → 队员 → 设备；汇总条显示 Admin **整体合规率**或经理**团队合规率**；默认卡片突出 **90% 合规线**。看重点三档只改顺序与主指标：**看健康** / **该催谁**（待跟进多的在前，标「催 n」不是金牌）/ **跟进成效**（唤醒率）；业绩与风险数字仍可下钻。队员行合并账号 + 同名未绑定设备 |
| `/xlv/managers/[managerKey]` | 经理下队员名单；`?status=` 考核筛选；`?sort=compliance\|pending\|wake_rate` |
| `/xlv/managers/.../staff/[staffKey]` | 队员设备列表（队员底栏「设备」同页）：已达标 / **考核中**（近日有动销，不含沉睡/单笔沉默）+ 沉睡类筛选；**不展示无效**；沉睡 / 单笔沉默卡片右侧显示 **待回访 / 已回访** |
...
| P2 | 考核中且剩余 ≤15 天（政策窗口至 2027 年 3 月） |
...
**期初**：上传 `微信盒子库存.xlsx`（`设备SN | 渠道 | 所属经理 | 作业员`）；**须先导入 SN 归属表**。按 SN 是否在归属表内：在表内 → `deployed`（已铺设）；表外且无作业员 → `manager_stock`；表外且有作业员 → `sales_stock`（队员库存）。支持 **dry-run 预览**。**默认跳过已有库存账**（`skipExisting=1`，不覆盖已铺设 / 经理库 / 队员库，便于只补未建账 SN）；取消勾选后同 SN 再次导入会覆盖（可只上传需更正的行，如改经理名）。期初表「所属经理」与运营归属不一致时，库存账以期初为准、合规率仍以运营表经理为准。
...
| `XlvDeviceRecord` | 小绿盒设备主表（SN 唯一）；含经理/作业员姓名与可选 `userId` 挂靠、沉睡指标、首末笔日期、**`relocatedAt`（换商户后考核起算）**；回访字段 `followUpDone` / `followUpNote` / `followUpAt` / `followUpById` / `followUpConnectStatus` / `followUpFlags` / `followUpPhotoUrls`（重导保留内容；下一日运营快照仍沉睡时仅重开 `followUpDone`） |
...
2. **① 运营原始表** — 按 SN + 统计日期写快照与最新指标。微信运营导出（统计日期 + 累计*）或**乐刷渠道原表**（保留 `0902_公司名.xlsx` 文件名，「累积活跃用户数」）都可直接传，不用改表头
...
| 表 | 必填列 | 说明 |
|---|---|---|
| 原始表 | 设备 SN；统计日期（乐刷可用文件名 `0902_` 补） | 微信运营导出或乐刷原表 |
...
### 2026-09-08（达标按已铺设、不按拓展月切开 · **已部署** `159ff70`）

- [x] **口径**：已铺设上月未达标，当月继续冲 20 用户 + 300 笔；已达标/达标率分母 = 全部已铺设，不再用本月首笔
- [x] **列表**：设备页默认全部已铺设；点「本月拓展」才看该月新铺
- [x] **日期**：拓展按中国日历日；跟进/唤醒按北京时间当天，避免 UTC 把 9 月跟进算进 8 月
- [x] **队员绩效**：考核中/已达标看名下全部已铺设；本月新铺单独统计

### 2026-09-03（小绿盒首末笔日期补回 · **已部署** `92c626d`）

- [x] **现象**：Excel 有首笔/末笔（如澜天便利店 2026-08-31）和累计 16 笔，详情却显示「—」、考核未起算、趋势空白
- [x] **导入**：运营表/归属表更新时，空的首末笔不再把库里已有日期写成 NULL
- [x] **详情**：缺日期时用快照或统计日推断并写回；仅一条截面且无当日列时，用累计画装机日趋势

### 2026-09-03（设备详情考核进度文案 · **已部署** `0783bd6`）

- [x] **冲突**：累计已有交易时，考核进度仍写「暂无首笔交易，尚未进入考核窗口」，与「考核中」徽章打架
- [x] **文案**：无首笔且有累计 → 说明运营表缺日期、暂无法按月考核；无交易 → 尚未产生交易、从首笔当月起算；均记考核中（尚未达标）

### 2026-09-01（小绿盒考核窗口延至 2027-03 · **已部署** `5a3250d`）

- [x] **政策**：由「装机后最多两个自然月」改为 **2027 年 3 月（含）前任意自然月** 达到 20 用户 + 300 笔即可；过完该月仍未达标才判无效
- [x] **规则**：`xlv-rules.ts` 逐月检查装机月至 `min(当前月, 2027-03)`；P2 剩余天数改按 2027-03 月底计算（当前不会进「考核将到期」）
- [x] **展示**：设备详情考核表列出已开始的各自然月；缺口文案不再写「次月重计」
- [x] **回填**：部署引导重算未评估设备 + 旧两月窗口下的 `invalid`（多数会回到考核中）

### 2026-09-01（设备页六宫格按月切换 · **已部署** `5a3250d`）

- [x] **月份**：`/xlv/alerts` 与回访情况同一套本月/上月控件；拓展、**已达标**、达标率、唤醒率跟所选区间（已达标 = 该月首笔 ∩ 已考核达标，9 月初看本月是 0，切上月才是 8 月成绩）
- [x] **空月**：所选月没有首笔设备时达标率显示 **—**（不再把 0/0 画成 0%）

### 2026-08-25（移机后考核不用旧店残量充新店进度 · **已部署** `96b682c`）

- [x] **现象**：绿居行卡片已写「新店暂无收款」，考核仍写「差 12 用户 · 差 292 笔」（20−8 / 300−8），把微信 SN 残量当成新店已收
- [x] **规则**：末笔早于考核起算时，当月成绩视为尚未收款，缺口文案为「装机月尚未收款 · 目标 20 用户 + 300 笔」

### 2026-08-25（移机后不展示旧店交易 · SN 能搜到 · F1 自动推运维小群 · **已部署** `c50cbe8`）

- [x] **旧店交易**：末笔早于考核起算时，新店卡片/详情不展示末笔、累计、沉睡徽章；交易趋势只保留考核起算及之后（07-21 等旧店柱去掉）
- [x] **搜索**：`URLSearchParams` 不编码 `*`，小绿盒 SN 写入 `?q=` 会被截断；改为 `%2A`。有搜索词时在角色可见范围内搜全库（含库存），SN 精确匹配
- [x] **F1**：`push-and-deploy.sh` 部署成功后自动 `ops-alert.sh` 运维小群；日常 F1 不要用 `post-deploy-notify.sh` 代替（会再推负责人群汇总）

### 2026-08-25（小绿盒移机后重开考核窗口 · **已部署** `1b26649`）

- [x] **现象**：同 SN 8/22 移机到新店后，卡片已标「移机到新店 / 绿居行商店」，但考核仍按旧店首笔（7/21）算「次月重计 / 考核将到期」；累计已是微信解绑旧店后的残量（如 8 用户 · 8 笔）
- [x] **规则**：`relocatedAt` 写入设备表；考核装机月 / 两月窗口 / P2 剩余天数以 `max(首笔, 移机日)` 起算；移机后不用终身累计直接判达标
- [x] **导入**：SN 归属换商户名时写入 `relocatedAt`（统计日）；部署引导从「SN归属换商铺设」流水回填并重算考核
- [x] **展示**：移机设备列表/详情「考核起算」为换商户日；本月拓展仍按微信首笔，不因移机重复计入

### 2026-08-25（小绿盒降内存 · 列表/看板不再拉全量快照 · **已部署** `03c79a3`）

- [x] **根因**：白天 RSS 贴在 1120–1180MB（阈值 1200）；看板/待办/分公司唤醒仍 `loadXlvSnapshotMap` 全历史；今日待办、设备列表、队员下钻同样为缺口文案拉全表快照；导入后逐台重算考核。17:10 连续超阈自动重启。
- [x] **列表口径**：今日待办、设备分页、队员设备改用落库 `qualificationStatus` + `xlvStoredQualificationGap`，与回访列表一致
- [x] **唤醒统计**：团队看板 / 分公司看板改为 `loadXlvSnapshotMapAfterFollowUp`（只拉跟进日之后）
- [x] **每日绩效 / 队员月绩**：只拉所选日期范围内关单/拓展，不再扫历史全量
- [x] **导入**：解析后立刻丢掉 Excel buffer；考核重算按 80 台一批拉快照（`statDateFrom=firstTxnDate`）再 `updateMany`
- [x] **缓存/并发**：`board-cache` 最多 3 条、RSS≥900MB 不写入；RSS≥1000MB 时重查询改串行
- [x] **容器**：仍 2G / 堆 1536MB / 巡检 1200MB；靠少分配而不是加配额

### 2026-08-25（导入失败必提醒 · 移机展示 · 考核将到期可下钻 · **已部署** `f6a0468`）
...
- [x] **生产构建**：设备详情补 `XLV_WITHDRAW_IMPORT_ENABLED` 导入；缺导入时 docker `next build` 类型检查失败，`58dc634` 未能上线
...
- [x] **Daily / 导出**：快照只拉 `followUpAt` 之后；2026-08-25 起每日绩效只统计所选日期范围内关单（与 pulse 同口径）
...
同机约 **8G** 内存（已从 3.6G 升配），跑 ali / unicom / hk 等多服务。大表导入与看板全量快照曾把 app RSS 顶到 1.5–1.9GB，触发巡检自动重启。

| 措施 | 说明 |
|---|---|
| Swap 2G | 主机已挂载 `/swapfile`，开机自动启用 |
| 容器内存上限 | `docker-compose.prod.yml`：app **2G**、postgres 768M；超限 OOM Kill 后 Docker 自动重启 |
| Node 堆上限 | `NODE_OPTIONS=--max-old-space-size=1536 --expose-gc`（须小于 app 容器上限；解析完可主动回收） |
...
| **看板降内存** | 列表/待办/队员设备用落库达标状态，不拉历史快照；看板/分公司唤醒只拉跟进日之后的快照；每日绩效只拉所选日期范围内关单；服务端缓存最多 3 条且 RSS≥900MB 不写入；RSS≥1000MB 时重查询改串行 |
...
- 主机已升到约 **8G**；app 容器仍限 **2G**，靠查询降内存而不是继续加配额
...
| SN 归属内存（2026-08-28） | Excel 稀疏解析并收窄 `!ref`；归属按 400 台一批；库存批量 SQL + `createMany` 流水；考核只重算新建/换商户。合表不必按行数拆 |
| 资源 | app 容器 **2G**，Node 堆 **1536MB** + `--expose-gc` |
...
src/services/import/xlv-excel-parser.ts
