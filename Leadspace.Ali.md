...
**最后更新**：2026-09-04（小绿盒：SN 归属导入金额小数不再整表失败 · **已部署** `8978e85`）
...
| 小绿盒双表导入 | 管理员 `/xlv/admin/import`：**先**运营原始表（多日快照 + 全量指标）→ **再**组织名册 → **再** SN 归属；后台任务 + 轮询；导入完成展示**摘要**（列匹配、日期范围、重复合并、未匹配姓名） |
| 小绿盒 SN 归属导入 | 按 **400 台一批**写设备/库存；Excel **只解析有内容的格子**（整列格式刷到百万行时不再按矩形分配内存）；考核只重算新建/换商户。合表可整份上传，**不是** 1400 行上限。批量 UPDATE 的 VALUES **金额列须显式 float8**（用户/笔数为 int）：首行金额为 0 时 Postgres 否则把整列推断成 integer，`20942.8` 报 `22P02`，整表失败须重传。2026-08-28 曾因 dense 解析 + 逐台写库存把 2GB 容器 OOM Kill，表现为「服务重启导致导入中断」 |
...
| 小绿盒考核 | 自然月增量：**+20 用户 + 300 笔**（两项须同时满足）；首笔交易月为装机月；**2027 年 3 月（含）前任意自然月达标即可**，过完 2027-03 仍未达标才 `invalid`；**月成绩 = 该自然月内最后截面「累计用户/笔数」减月前基线**（对齐微信运营表累计列；交易趋势图仍按收款日展示逐日笔数）；装机月无截面时可回退设备累计；状态 `qualified` / `in_progress` / `invalid`（`xlv-rules.ts`）。**同 SN 换商户（移机）后考核窗口从 `relocatedAt` 起算**，不再沿用旧商户首笔；微信解绑后的累计只计入新店窗口。本月拓展仍按微信 `firstTxnDate`。运营表漏填首末笔时：导入**不得用空值覆盖**已有日期；打开设备详情用快照/统计日补回（有累计则首末笔可落到该统计日）。仅一条截面且无「当日」列时，趋势用累计当作装机日活动 |
...
| 小绿盒看板 | `/xlv/alerts` 六宫格；**拓展 / 已达标 / 达标率 / 唤醒率按所选月份**（本月/上月；已达标 = 该月首笔里已考核达标，不是累计全库；无拓展时达标率显示 —）；沉睡 / 单笔沉默为当前状态。`/xlv/board` 经理排行；**`/xlv/admin/companies` 分公司排名看板**（Admin，按沉睡排序，末尾未归属/待定） |
