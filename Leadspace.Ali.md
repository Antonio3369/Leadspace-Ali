...
**最后更新**：2026-09-08（小绿盒看板：全队合计与本人合规分开读 · **已部署** `__HASH__`）
...
| 小绿盒看板 | `/xlv/alerts` 六宫格：**已达标 / 达标率按全部已铺设**（往月未达标当月继续考核，不限首笔月）；**本月/上月拓展**只统计该月新铺；沉睡 / 单笔沉默为当前状态。列表**默认全部已铺设**，点「本月拓展」才按首笔月筛。日期区间与中国日历日对齐（跟进按北京时间）。**考核中**卡片展示当月成绩 + 距目标缺口，至今累计只在详情。`/xlv/board` 顶栏是**全队/全部经理合计**（合规台数/已铺设、距 90% 差几台）；**终身已达标 ≠ 合规**（考核中仍在用也算合规）。下面每人只算自己的设备，卡片写「本人 n/m 台合规」。看重点三档：**看健康** / **该催谁** / **跟进成效**。队员行合并账号 + 同名未绑定。**`/xlv/admin/companies` 分公司排名看板** |
...
| `/xlv/board` | **团队看板**：顶栏 **全队/全部经理合计**（合规台数/已铺设，距 90% 差几台；终身已达标单独一行，不等于合规）。名单每人只算自己的设备（「本人 n/m 台合规」）。看重点：**看健康** / **该催谁** / **跟进成效**。队员行合并账号 + 同名未绑定 |
