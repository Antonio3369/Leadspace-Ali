...
**最后更新**：2026-09-05（小绿盒：乐刷原表可直接导① + 笔数/金额不再串列 · **已部署** `b19f91b`）
...
| 小绿盒看板 | `/xlv/alerts` 六宫格；**拓展 / 已达标 / 达标率 / 唤醒率按所选月份**（本月/上月；已达标 = 该月首笔里已考核达标，不是累计全库；无拓展时达标率显示 —）；沉睡 / 单笔沉默为当前状态。设备列表本月/上月**默认按首笔日期筛选**（`expand=all` 才看全部）；宫格区间与库内 UTC 日历日对齐。`/xlv/board` 经理排行；**`/xlv/admin/companies` 分公司排名看板**（Admin，按沉睡排序，末尾未归属/待定） |
