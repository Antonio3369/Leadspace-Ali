...
**最后更新**：2026-08-18（小绿盒：设备库存 Phase 1、库存看板 + 合规率补货决策、团队看板与物流账分离 · **未部署**）
...
| 小绿盒看板 | `/xlv/alerts` 顶部六宫格（本月拓展 / 已达标 / 达标率 / 单笔沉默 / 沉睡 / 唤醒率）；`/xlv/board` 经理排行（**纯运营考核**：合规 / 业绩 / 风险；**不含库存**），队员行展示业绩 / 风险 / 本月跟进与唤醒；三级下钻经理 → 队员 → 设备 |
| 小绿盒设备库存 | `/xlv/admin/inventory`（Admin）/ `/xlv/inventory`（经理）：**库存看板**（收到 / 已铺设 / 剩余 / 铺设率 + **合规率** 供补货决策）；入库 / 划拨 / 期初 / 移机撤机；与团队看板分离，见 §6.2d |
...
| 路径 | 角色 | 功能 |
|---|---|---|
| `/xlv/admin/inventory` | DIRECTOR | **库存看板**（默认 Tab）、期初盘点、Admin 入库、划拨经理、移机撤机、待收货确认 |
| `/xlv/inventory` | MANAGER | **库存看板**、待收货确认、分给队员、移机撤机 |
...
**撤机判定（增量 A）**：**仅以运营维护的移机明细 Excel 为准**；未上传移机表时不自动撤机；每次上传 = 本批新撤机 SN；同 SN 多行取 **进件日期最新** 一条。撤机后回到 **铺设归属人库存**（经理铺 → `manager_stock`；队员铺 → `sales_stock`）。经理 **只能撤本团队** SN；DIRECTOR 可撤全部。

**入库**：仅 Admin — Excel 入库（列：`设备SN`，可选 `渠道`）或 Excel 划拨经理（`设备SN` + `所属经理`）→ 经理 PC **确认收货** 后入 `manager_stock`。经理→队员：**无需队员确认**，上传 `设备SN` + `作业员` 即入 `sales_stock`。
...
**与运营导入协作**：建议顺序 ① 库存变动 ② 运营原始表 ③ 名册 ④ SN 归属（逐步弱化）。库存未建账的 SN（`inventory` 为空）在 Phase 1 **仍视为已铺设**，兼容上线前数据；期初/撤机后 **`deployed` 以外状态** 的设备从默认列表、沉睡预警、今日待办等运营视图中排除（`xlv-scope.ts`）。**移机撤机只改库存账**，不自动改 SN 归属表。

**Phase 2（未做）**：`XlvDeviceDeployment` 铺设轮次、详情页历史时间线、二次铺设自动开新轮。

实现：`src/services/xlv/inventory/*`、`src/lib/xlv-inventory.ts`；模型 `XlvInventoryDevice` / `XlvInventoryTransfer`。
...
| **小绿盒 5 · 库存 Phase 1** | ~~入库/划拨/移机撤机/期初盘点、库存看板、合规率补货列、团队看板去库存~~（已完成，见 §6.2d；**未部署**）；Phase 2 铺设轮次与详情历史 |
