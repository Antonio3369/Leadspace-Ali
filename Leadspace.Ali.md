...
**最后更新**：2026-08-25（期初导入默认可跳过已有库存账 · **已部署** `9ccf023`）
...
生产 https://ali.orblead.com。已上线：N7 底栏、队员开号、V1 关单、设备挂靠、设备搜索；**微信小绿盒**（沉睡回访、今日待办、关单回告、运维告警运维小群、**分公司排名看板**、企微分流；**导入失败/中断推运维小群**、导入页三表成功失败一目了然、**移机到新店**、考核将到期点开看完整名单 · `f6a0468`；**列表/看板不再拉全量快照** · `03c79a3`；**移机后考核从换商户日起算** · `1b26649`；**移机后不展示旧店交易 / SN 搜索编码 / F1 部署成功推运维小群** · `c50cbe8`；**移机后考核不用旧店残量** · `96b682c`；**期初默认可跳过已有库存账** · `9ccf023`）。  
