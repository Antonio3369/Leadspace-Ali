...
| 小绿盒运维企微 | **导入卡死 / 站点不可用 / 容器未运行 / 内存过高 / 自动重启** → **个人企微应用**「LEADspace 运维」（`WECOM_*`）；业务群仍收跟进/撤机/导入成功，**不收内存**；cron 每 10 分钟；连续两次超 1200MB（约 20 分钟）且无导入才自动重启 app；每天 03:00 定时重启；见 §15.6 |
