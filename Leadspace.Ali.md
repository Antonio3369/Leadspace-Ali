...
**最后更新**：2026-09-03（设备详情考核进度：缺首笔日期时不再写「尚未进入考核窗口」 · **已部署** `0783bd6`）
...
| 小绿盒考核 | 自然月增量：**+20 用户 + 300 笔**（两项须同时满足）；首笔交易月为装机月；**2027 年 3 月（含）前任意自然月达标即可**，过完 2027-03 仍未达标才 `invalid`；**月成绩 = 该自然月内最后截面「累计用户/笔数」减月前基线**（对齐微信运营表累计列；交易趋势图仍按收款日展示逐日笔数）；装机月无截面时可回退设备累计；状态 `qualified` / `in_progress` / `invalid`（`xlv-rules.ts`）。**同 SN 换商户（移机）后考核窗口从 `relocatedAt` 起算**，不再沿用旧商户首笔；微信解绑后的累计只计入新店窗口。本月拓展仍按微信 `firstTxnDate`。无首笔日期时详情「考核进度」：**有累计交易**说明运营表缺日期、暂无法按月考核；**无交易**写尚未产生交易、从首笔当月起算；均仍记「考核中」（尚未达标），**不写「尚未进入考核窗口」** |
