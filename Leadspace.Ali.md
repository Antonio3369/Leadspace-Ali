...
| 小绿盒考核 | 自然月增量：**+20 用户 + 300 笔**（两项须同时满足）；首笔交易月为装机月，最多考核两个自然月；**月成绩 = 该自然月内最后截面「累计用户/笔数」减月前基线**（对齐微信运营表累计列；交易趋势图仍按收款日展示逐日笔数）；装机月无截面时可回退设备累计；状态 `qualified` / `in_progress` / `invalid`（`xlv-rules.ts`）。**同 SN 换商户（移机）后考核窗口从 `relocatedAt` 起算**，不再沿用旧商户首笔；微信解绑后的累计只计入新店窗口。本月拓展仍按微信 `firstTxnDate` |
