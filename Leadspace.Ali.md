...
**最后更新**：2026-09-05（小绿盒：考核中列表改当月成绩+缺口 · **已部署** `541d8e2`）
...
| 小绿盒看板 | `/xlv/alerts` 六宫格；**拓展 / 已达标 / 达标率 / 唤醒率按所选月份**（本月/上月；已达标 = 该月首笔里已考核达标，不是累计全库；无拓展时达标率显示 —）；沉睡 / 单笔沉默为当前状态。设备列表本月/上月**默认按首笔日期筛选**（`expand=all` 才看全部）；宫格区间与库内 UTC 日历日对齐。**考核中**卡片展示当月（或装机月）用户/笔数 + 距目标缺口，**至今累计只在详情**。`/xlv/board` 经理排行；**`/xlv/admin/companies` 分公司排名看板**（Admin，按沉睡排序，末尾未归属/待定） |
