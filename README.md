...
登录后先选择业务线：**小蓝环**（完整看板）、**支付宝 N7**（今日待办 / 达标跟进 / 数据看板 / 处理状态）或 **微信小绿盒**（沉睡预警 / 考核看板 / 双表导入）。产品约定与路由详见 [Leadspace.Ali.md](./Leadspace.Ali.md)（**最后更新 2026-08-07**）。
...
- 登录后业务选择页：小蓝环 `/xlh`、支付宝 N7 `/n7`、微信小绿盒 `/xlv`
...
- 双表 Excel 导入（`/xlv/admin/import`）：**先**运营原始表 → **再**人员归属表；导入完成展示摘要
- **沉睡预警** `/xlv`：单笔沉默 / 沉睡 / 正常活跃；考核状态筛选（已达标 / 考核中 / 无效用户）
...
| 阶段 | 内容 |
|------|------|
| 小绿盒 2 | 导入未匹配报告、未挂靠设备列表、人员 relink、轻量归属维护 |
| 小绿盒 3 | 沉睡回访跟进、今日待办、每日绩效、Excel 导出 |
