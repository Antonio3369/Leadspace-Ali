...
登录入口分两路：**支付宝业务**（小蓝环 + N7，共用 `User`，`/login` → `/alipay`）与 **微信小绿盒**（独立 `XlvMemberAccount`，`/login/xlv`）。未登录访问 `/` 先选平台。产品约定与路由详见 [Leadspace.Ali.md](./Leadspace.Ali.md)（**最后更新 2026-08-08**）。
...
- **今日待办** `/xlv`：P0 优先催办 / P1 一般沉睡 / P2 考核将到期；快捷卡链到沉睡回访台账
- **沉睡预警** `/xlv/alerts`：单笔沉默 / 沉睡 / 正常活跃；考核状态筛选（已达标 / 考核中 / 无效用户）
- **沉睡回访** `/xlv/follow-up`（无侧栏入口）：跟进需跟进图（至少一张）；支持导出 Excel
- **回访情况** `/xlv/daily`：回访跟进 / 已唤醒 / 仍沉睡；按**跟进日**筛选；经理看队员、负责人看经理
- **团队看板** `/xlv/board`：经理 → 队员 → 设备；排行行指标为纯文字彩色（可点进筛选）；队员名可进**月绩效**；剩余库存单独摘要（`总数/库存`），不进经理排行
- **队员月绩效** `/xlv/managers/.../performance`：拓展/达标（首笔交易月）、回访跟进（跟进日）、唤醒
- 设备详情：考核进度（自然月表格 + 当前月高亮，无重复摘要卡）+ **交易趋势**（仅有收款日）；沉睡类可跟进/重开回访；商户名与 SN 可复制
- 自然月考核：首笔交易月起最多两月，每月 **20 用户且 300 笔**（须同时满足）
- **人员挂靠** `/xlv/admin/attribution`：未匹配姓名报告、未挂靠设备、批量重挂、单台修正
