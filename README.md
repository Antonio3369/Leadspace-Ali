...
登录后先选择业务线：**小蓝环**（完整看板）、**支付宝 N7**（今日待办 / 达标跟进 / 数据看板 / 处理状态）或 **微信小绿盒**（今日待办 / 沉睡预警 / 考核看板 / 双表导入）。产品约定与路由详见 [Leadspace.Ali.md](./Leadspace.Ali.md)（**最后更新 2026-08-08**）。
...
- **沉睡回访** `/xlv/follow-up`（无侧栏入口）：关单需跟进图（至少一张）；支持导出 Excel
- **每日绩效** `/xlv/daily`：回访关单 / 已唤醒 / 仍沉睡；经理看队员、负责人看经理
- **团队看板** `/xlv/board`：经理 → 队员 → 设备；剩余库存单独摘要（`总数/库存`），不进经理排行
- 设备详情：考核进度 + 多日快照趋势图；沉睡类可关单/重开回访；商户名与 SN 可复制
- 自然月考核：首笔交易月起最多两月，每月 +20 用户 + 300 笔
...
| 小绿盒 3 | ~~沉睡回访、今日待办、每日绩效、Excel 导出、唤醒统计~~（已完成） |
