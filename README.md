...
登录入口分两路：**支付宝业务**（小蓝环 + N7，共用 `User`，`/login` → `/alipay`）与 **微信小绿盒**（独立 `XlvMemberAccount`，`/login/xlv`）。未登录访问 `/` 先选平台。产品约定与路由详见 [Leadspace.Ali.md](./Leadspace.Ali.md)（**最后更新 2026-08-13**）。
...
- **今日待办** `/xlv`：P0 优先催办 / P1 疑似沉睡（2天≤沉睡＜7天）/ P2 考核将到期；顶栏 **「系统通知」** → `/xlv/notifications`（**队员已跟进** + **撤机通知** 两分区）
- **设备** `/xlv/alerts`：顶部六宫格（本月拓展 / 已达标 / 达标率 / 单笔沉默 / 沉睡 / 唤醒率）；负责人选经理后队员下拉仅该团队
- **沉睡回访** `/xlv/follow-up`（无侧栏入口）：待回访 / 已回访 / 全部；`priority=P0|P1` 与待办对齐；跟进需跟进图；关单通知经理 + 抄送 `admin`
- **回访情况** `/xlv/daily`：回访跟进 / 已唤醒 / 仍沉睡；按**跟进日**筛选
- **团队看板** `/xlv/board`：经理 → 队员 → 设备；汇总条及人员卡片均突出 90% 动态合规线（已达标、考核中且活跃、跟进后已唤醒且当前仍活跃）；仅跟进不计合规，再次沉睡即退出
- **队员设备** `/xlv/managers/.../staff/...`（队员底栏「设备」）：已达标 / 考核中 / 沉睡类筛选；沉睡设备卡片显示回访状态
