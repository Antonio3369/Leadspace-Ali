...
- 登录后业务选择页：小蓝环 `/xlh`、支付宝 N7 `/n7`、微信小绿盒 `/xlv`（今日待办首页）
...
- 考核表 Excel 导入（`/n7/admin/import`）
- **今日待办** `/n7`：系统催办（P0 未关单）；未处理等为数字卡入口；复盘进 `/n7/board`；**按考核期**列名单（非注册月）
- **达标跟进** `/n7/follow-up` · 队员明细 · 设备详情；支持搜门店/SN/手机（`q`，跨月）
- **数据看板** `/n7/board`：拓展/达标按**注册日期**（本月/上月）
- **处理状态**：与考核「待跟进」独立；详情点选即保存；列表可「标已处理」；备注进详情
- 紧急度界面用人话（剩余≤2天 / 无动销 / 行为未齐 / 一般预警）；内部仍用 P0–P3
...
- 设备详情：考核进度 → 沉睡回访 → 交易趋势；状态文案统一（无「正常」）
- 手机底栏：**经理** 待办·设备·团队·回访·我的；**队员** 待办·设备·回访·我的（4 键）
- **人员挂靠** `/xlv/admin/attribution`
...
| 阶段 | 内容 |
|------|------|
| 小绿盒 2 | ~~未匹配报告、未挂靠设备、relink、轻量归属~~（已完成） |
| 小绿盒 3 | ~~沉睡回访、今日待办、回访情况、Excel 导出、唤醒统计~~（已完成） |
| 小绿盒 4 | ~~平台分流、独立开号、队员月绩效、考核按收款日、交易趋势~~（已完成） |
| 小绿盒 5 | ~~关单回告经理、状态文案统一、队员底栏 4 键~~（已完成） |
| N7 | 业务员端写入处理状态；空态/移动端体验 |
...
src/
├── app/
│   ├── (dashboard)/     # / 业务选择；/xlh 小蓝环；/n7 N7；/xlv 小绿盒
│   ├── api/             # API 路由
│   └── login/           # 登录页
