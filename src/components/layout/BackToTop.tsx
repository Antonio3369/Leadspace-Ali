...
export function BackToTop() {
  ...
  useEffect(() => {
    ...
    const syncBottomTabs = () =>
      setBottomTabs(document.documentElement.dataset.bottomTabs === "1");
    syncBottomTabs();
    const obs = new MutationObserver(syncBottomTabs);
    obs.observe(document.documentElement, {
      attributes: true,
      attributeFilter: ["data-bottom-tabs"],
    });
  ...
  return (
    <button
      ...
      aria-label="返回顶部"
      onClick={handleClick}
      className={`fixed z-[80] right-4 md:right-6 ${mobileBottom} md:bottom-6 flex h-11 w-11 items-center justify-center rounded-full border border-[#e2e8f0] bg-white/95 text-[#334155] shadow-[0_8px_24px_rgba(15,23,42,0.12)] backdrop-blur-sm transition-all duration-200 hover:border-[#cbd5e1] hover:text-[#0f172a] active:scale-95 ${
        visible
