...
export function N7FollowUpCloseForm({
  ...
  const [connectStatus, setConnectStatus] =
    useState<N7FollowUpConnectStatus | null>(
      initialConnect === "connected" || initialConnect === "not_connected"
        ? initialConnect
        : null
    );
  const [flags, setFlags] = useState<N7FollowUpFlag[]>(
    initialFlags.filter(
      (f): f is N7FollowUpFlag => f === "unwilling" || f === "promised_use"
    )
  );
  ...
  async function onPickFiles(files: FileList | null) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const saved = await uploadN7FollowUpPhoto(deviceSn, file);
  ...
  async function submitDone() {
    ...
    try {
      ...
      if (acknowledgeOnly) {
        setMessage("已标记已知悉");
      } else if (json.managerNotified) {
        setMessage("关单成功，经理将收到提醒通知");
      } else {
        setMessage("关单成功");
      }
    } catch (err) {
