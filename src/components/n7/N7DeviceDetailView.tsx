...
import { N7FollowUpCloseForm } from "@/components/n7/N7FollowUpCloseForm";
...
export function N7DeviceDetailView({ sn }: { sn: string }) {
  ...
  function applyFollowResult(json: N7FollowUpPatchResult) {
    setData((prev) =>
      prev
        ? {
            ...prev,
  ...
  const hopeless =
    data != null &&
    (typeof data.hopeless === "boolean"
      ? data.hopeless
      : isN7TimeHopeless(data));
    ...
    <PageShell>
      ...
      {data && (
        <div className="max-w-2xl space-y-4">
          {!data.isQualified && (
            <div className="rounded-[14px] border border-[#eef2f7] bg-white p-5 shadow-sm text-sm">
              <N7FollowUpCloseForm
                key={`${data.deviceSn}-${data.followUpDone}-${data.followUpAt ?? ""}`}
                deviceSn={data.deviceSn}
                done={data.followUpDone}
                note={data.followUpNote ?? ""}
                connectStatus={data.followUpConnectStatus}
                flags={data.followUpFlags ?? []}
