...
export function N7NotificationsView() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(N7_NOTIFICATIONS_CHANGED, onChanged);
    return () => {
      window.removeEventListener("pageshow", onPageShow);
      document.removeEventListener("visibilitychange", onVisible);
      window.removeEventListener(N7_NOTIFICATIONS_CHANGED, onChanged);
    };
  }, [load]);
  ...
  async function markAll() {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/n7/notifications", {
        method: "PATCH",
  ...
  function openItem(item: NotifItem) {
    ...
    emitN7NotificationsChanged();
    void fetch("/api/n7/notifications", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <PageShell>
      ...
      <ul className="space-y-2 max-w-2xl">
        {items.map((item) => (
          <li key={item.id}>
            <Link
              href={n7Path(`/devices/${encodeURIComponent(item.deviceSn)}`)}
