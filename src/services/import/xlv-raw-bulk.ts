  ...
  for (let i = 0; i < rows.length; i += DEVICE_UPDATE_CHUNK) {
    ...
    await db.$executeRaw`
      ...
      SET
        ...
        "lastTxnDate" = COALESCE(NULLIF(v."lastTxnDate", '')::timestamptz, d."lastTxnDate"),
        "sleepDays" = v."sleepDays"::integer,
        "isActivated" = v."isActivated"::boolean,
        "firstTxnDate" = COALESCE(NULLIF(v."firstTxnDate", '')::timestamptz, d."firstTxnDate"),
  ...
  for (let i = 0; i < updates.length; i += ASSIGNMENT_UPDATE_CHUNK) {
    ...
    await db.$executeRaw`
      ...
      SET
        ...
        "isActivated" = v."isActivated"::boolean,
        "firstTxnDate" = COALESCE(NULLIF(v."firstTxnDate", '')::timestamptz, d."firstTxnDate"),
        "relocatedAt" = COALESCE(NULLIF(v."relocatedAt", '')::timestamptz, d."relocatedAt"),
        "importBatchId" = NULLIF(v."importBatchId", '')::text,
        "updatedAt" = NOW()
      FROM (VALUES ${Prisma.join(
        chunk.map(
