...
import { submitN7FollowUpReview } from "@/services/n7/follow-up-review";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const updated = await submitN7FollowUpReview(
      user,
      deviceSn,
      body.reviewNote
    );
    ...
    if (/Unknown argument|does not exist|followUpReview/i.test(message)) {
      console.error("[n7 follow-up review]", message);
      return NextResponse.json(
        {
          error:
            "关单反馈字段未就绪：请重启本地开发服务（或生产环境执行 prisma db push 后重新部署）",
        },
        { status: 500 }
      );
    }
